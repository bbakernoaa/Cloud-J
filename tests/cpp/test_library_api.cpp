...
void test_cross_section_interpolation() {
    // 1-point
    double x1 = CloudJ::CrossSections::interpolate(250.0, 200.0, 1.5, 0.0, 0.0, 0.0, 0.0, 1);
    assert(std::abs(x1 - 1.5) < 1e-10);

    // 2-point
    double x2 = CloudJ::CrossSections::interpolate(250.0, 200.0, 1.0, 300.0, 2.0, 0.0, 0.0, 2);
    assert(std::abs(x2 - 1.5) < 1e-10);
    
    // 2-point clamped lower
    double x2_low = CloudJ::CrossSections::interpolate(150.0, 200.0, 1.0, 300.0, 2.0, 0.0, 0.0, 2);
    assert(std::abs(x2_low - 1.0) < 1e-10);

    // 2-point clamped upper
    double x2_high = CloudJ::CrossSections::interpolate(350.0, 200.0, 1.0, 300.0, 2.0, 0.0, 0.0, 2);
    assert(std::abs(x2_high - 2.0) < 1e-10);

    // 3-point (lower half)
    double x3_low = CloudJ::CrossSections::interpolate(250.0, 200.0, 1.0, 300.0, 2.0, 400.0, 4.0, 3);
    assert(std::abs(x3_low - 1.5) < 1e-10);

    // 3-point (upper half)
    double x3_high = CloudJ::CrossSections::interpolate(350.0, 200.0, 1.0, 300.0, 2.0, 400.0, 4.0, 3);
...
void test_radiative_solver_and_gauss_gauss() {
    ...
    // Check manual LU block-diagonal tri-diagonal inversion math functions compiles
...
void test_photolysis_and_orchestrator() {
    ...
    CloudJ::OutputRates dark_rates = engine.calculate_photolysis_rates(profile, 100.0);
