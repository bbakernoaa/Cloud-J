...
int main(int argc, char *argv[]) {
  ...
  if (!infile.is_open()) {
    ...
    if (!infile.is_open()) {
      std::cerr
          << "Error: Could not open profile file tables/atmos_PTClds.dat\n";
      return 1;
    }
  }
  ...
  // We instantiate AtmosphericProfile with 57 layers (which has 58 boundary
  // nodes/levels)
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    if (i == 0 && run_benchmark) {
      auto start = std::chrono::high_resolution_clock::now();
      for (int j_bench = 0; j_bench < 1000; ++j_bench) {
        CloudJ::OutputRates rates =
            engine.calculate_photolysis_rates(profile, sza);
      }
      ...
      double elapsed =
          std::chrono::duration_cast<std::chrono::microseconds>(end - start)
              .count() /
          1000000.0;
      std::cout << "PURE C++ IN-MEMORY LOOP (1000 runs): " << elapsed
                << " seconds\n";
    }
